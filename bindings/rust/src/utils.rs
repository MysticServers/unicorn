...
use std::{cell::RefCell, collections::HashMap, println, ptr};
...
pub fn add_debug_prints_arm<D>(
    uc: &mut super::Unicorn<'_, D>,
    code_start: u64,
    code_end: u64,
    offset: u64,
) {
    ...
    let callback = Box::new(move |uc: &mut super::Unicorn<D>, addr: u64, size: u32| {
        ...
        let ip = uc
            .reg_read(RegisterARM::R12 as i32)
        ...
        println!(
            "$pc: {:#010x}",
            if addr < offset { addr } else { addr - offset }
        );
        println!("{}", ins);
    });
...
pub fn print_arm_registers<D>(uc: &mut super::Unicorn<'_, D>, lib_offset: u64) {
    let sp = uc
        .reg_read(RegisterARM::SP as i32)
        .expect("failed to read SP");
    let lr = uc
    ...
    let ip = uc
        ...
        .expect("failed to read ip");
    let pc = uc
        .reg_read(RegisterARM::PC as i32)
    ...
    let out = format!(
        "$pc: {:#010x}",
        if pc < lib_offset { pc } else { pc - lib_offset }
    )
    .blue();
    println!("{out}",);
}
...
pub fn vmmap<D>(uc: &mut super::Unicorn<'_, D>) {
    let regions = uc
        .mem_regions()
